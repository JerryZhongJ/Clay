...
- 倒计时
- 闹钟
- ~~潜意识时钟：每十秒发一次活动~~，不再发出统一的时钟信号。每个模块可以自己设置代码上计时器，自己控制时间。本来模块就是异步的。
...
注意域拥有固定大小(可配置,默认由LLM决定)。容量按活动格式化后的字符串长度计算。

**清空机制**:
- 当容量达到100%时触发清空
- 采用FIFO策略,默认清空50%(用户可配置)
- 清空操作异步执行,产生包含被清空活动的清空活动
- 被清空的活动暂存在内存中,供记忆模块持久化
