...
*注:事件也可称为活动(Activity)。我更青睐"活动",但本文档暂时使用"事件"。*
...
初步构想中，事件至少包含以下几种（不是互斥的分类）:

- **UnderConscious**:"潜意识"活动,不出现在注意域中,但会出现在事件流中
- **WakeUp**:当系统沉睡时,此类事件会将系统唤醒

**重要**:事件不是日志。事件是 AI "大脑"中产生的各种活动,而日志是代码行为。

### 事件存储

事件存储在两个位置:

1. **事件流**:所有产生的事件,包括注意域中的事件
2. **注意域**:部分事件,不包括 UnderConscious 事件,且经 System 筛选

## System 模块详解

System 模块是事件流、注意域的管理者,也是各模块的协调者。

### 管理其他模块

- 其他模块需要注册在 System 中
- 模块可以注册特定条件的事件
- 当遇到特定事件时,System 调用相应功能
...
不是所有事件都能进入注意域。System 对每个模块都有 focus 值,focus 值越高的模块可以准入更多事件。每个模块的事件按重要性排序,优先让重要事件进入注意域,其余只会 append 在事件流中。

**事件响应机制**:

当事件流出现被注册的事件时,System 需要响应并调用相应功能。

System 本身也是模块,它也有功能(如调整 focus 值),也可以注册在自己身上。

### 管理注意域

注意域拥有固定大小。当容量满时,需要清空至特定大小。

### 产生事件

System 本身的各种活动也会产生事件。例如,flush 注意域会产生"潜意识"事件。
